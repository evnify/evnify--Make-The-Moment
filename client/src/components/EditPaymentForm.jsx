...
const EditPaymentForm = (props) => {
    ...
    const { cart, userId, selectedPackage, selectedAddress, price } = props;
    ...
    const saveBooking = async (transaction) => {
        try {
            await axios.post(`${process.env.PUBLIC_URL}/api/bookings/savePayment`, {
            ...
            await axios.post(
                `${process.env.PUBLIC_URL}/api/bookings/editBookingById`,
                {
                    ...
                    cart,
                    amount : selectedPackage[0].amount + price,
                }
            );
    ...
    async function onToken(token) {
        const stripeToken = {
            token,
            amount: price,
            description: `Additional Payment for ${selectedPackage[0].packageType} Package by ${nameOnCard}`,
            email: email,
        };
        try {
            const response = await axios.post(
                `${process.env.PUBLIC_URL}/api/bookings/payment`,
                stripeToken
            );
