...
function MessageInsights() {
    ...
    const fetchMessages = async () => {
        try {
            ...
            setReadMessage(
                messageArray.filter((message) => message.status === "read")
                    .length
            );
            setUnreadMessage(
                messageArray.filter((message) => message.status === "unread")
                    .length,
                    console.log(messageArray.filter((message) => message.status === "unread"))
            );
            ...
            const filteredMessages = messageArray.filter((message) => {
                ...
                return (
                    messageDate >= currentWeekStart &&
                    messageDate <= currentWeekEnd
                );
            });
            ...
            messageArray.forEach((message) => {
                categoryCounts[message.category] =
                    (categoryCounts[message.category] || 0) + 1;
            });
